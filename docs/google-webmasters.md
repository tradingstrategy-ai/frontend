...
| Glossary → vault links                         | `getRelatedVaultLinks()` in `src/routes/glossary/related-vault-links.ts`                                                                                                                                                                                                                                                                                                                                                                                      |
...
5. **Glossary links** — the glossary terms that rank link to the matching vault pages (`hyperliquid-provider-vault` → HLP vault and Hyperliquid hub; `vault`, `erc-4626`, `erc-7540`, `apy`, `stablecoin` and `total-value-locked-tvl` → the matching hub).
6. **Structured data** — hub `ItemList` entries for the server-rendered rows; `dateModified` from the dataset.

Built and removed before release: a generated "top vaults" block on every hub, a stablecoin yield comparison table, "Similar <protocol> vaults" on vault pages, a "<name> APY and returns" heading and a `/vaults/methodology` page; see `docs/vault-listings.md`, "Search wording".
