...
| Hub findings and leader comparisons            | `getListingInsights()` in `src/lib/top-vaults/listing/insights.ts` (leader rules: yield vaults only, Low risk or safer, ≥ $100k TVL, ≥ 3 months, APY ≤ 100 %), rendered by `VaultHubInsights.svelte`; `/vaults/stablecoins` uses the same rules for its comparison                                                                                                                                                                                            |
...
5. **Page-specific findings** on every hub (APY leaders, median APY, largest curator or protocol, data date) and a leader comparison table; a "Best stablecoin yields right now" table on `/vaults/stablecoins`. The first live run named 600–850 % trading vaults and funds as the best USDC yield, so leaders are now yield vaults only (no pools, no trading-strategy tags — the risk rating is protocol risk and does not screen them out), rated Low risk or safer, with ≥ $100k TVL, ≥ 3 months of history and APY ≤ 100 %.
