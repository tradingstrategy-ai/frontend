...
| `noindex` for generated images                 | `handleResponseHeaders` in `src/hooks.server.ts` sets `X-Robots-Tag: noindex` on `/social-card/*` and `/metadata-logo/*`, including their redirects. Export-data pages and unnamed exchanges (`isUnknownExchangeName`) return `robots` from their loaders.                                                                                                                                                                                                    |
| Titles, descriptions, `og:image`               | `SocialCardMetaTags` (`titleParts` → `getPageTitle()` in `src/lib/helpers/seo.ts`, description cut by `getMetaDescription()`, default social image); every indexable template uses it. `tests/integration/head-meta.test.ts` asserts one canonical, one branded title, a 70–155 character description and one `og:image` on a sample URL of each template; `api/explorer/` in the static sitemap is the backend's own Swagger page and outside this contract. |
...
| Font layout shift                              | Metric-matched fallback faces in `src/lib/components/css/font-fallbacks.css` (render-blocking; see `docs/speed.md`); `src/lib/server/font-preload.ts` sends the `Link` preload headers on HTML responses only; `scripts/static-cache-control.js` gives the font files their one-year `Cache-Control` from `scripts/server.js`.                                                                                                                                |
...
| Vault hub titles and descriptions              | `getHubTitleParts()` and `getHubDescription()` in `src/lib/top-vaults/hub-seo.ts` — search wording ("Morpho vaults", "APY", "best USDC yield"), data-led descriptions; used by `/vaults` and the chain, protocol, stablecoin and curator hubs                                                                                                                                                                                                                 |
| Vault detail titles and generated descriptions | `getVaultTitleParts()` and `getGeneratedVaultDescription()` in `src/lib/top-vaults/vault-seo.ts`                                                                                                                                                                                                                                                                                                                                                              |
| Stablecoin yield comparison                    | `getStablecoinYieldComparison()` in `src/lib/top-vaults/listing/stablecoin-yields.ts` with the leader rules of `getYieldLeaders()` in `src/lib/top-vaults/listing/insights.ts` (yield vaults only, indexable, Low risk or safer, ≥ $100k TVL, ≥ 3 months, APY ≤ 100 %)                                                                                                                                                                                        |
| Internal links between vaults                  | `getSimilarVaults()` in `src/lib/top-vaults/similar-vaults.ts`; glossary term → vault links in `src/routes/glossary/related-vault-links.ts`                                                                                                                                                                                                                                                                                                                   |
| Ranking methodology                            | `/vaults/methodology`, linked from every listing                                                                                                                                                                                                                                                                                                                                                                                                              |
...
1. **Vault index hygiene.** `isVaultIndexable`: not indexable when blacklisted, the name is blank/placeholder or blocklisted, current _and_ peak TVL (in USD — `current_nav` is in denomination units) are confirmed below $5,000, or the protocol is unknown and the current TVL is below $5,000. Missing TVL or a missing USD rate keeps a page indexable — except on an unknown protocol, where only a confirmed current TVL keeps the page (a page that can name neither its protocol nor its size is thin by definition). Only an explicit rate, a USD denomination or the backend's `stablecoinish` flag count as a confirmed USD figure; an unrecognised denomination is never read as dollars. Dry run against the live dataset on 2026-09-17: **1,041 of 4,681 listed vaults (22 %) become `noindex`** — 896 unknown-protocol vaults whose TVL has left (peak ≥ $5k, now < $5k), 122 with never more than $5k, 20 placeholder names, 3 blocklisted names; 8 named vaults with no TVL data stay indexable. The sitemap shrinks from 5,280 to 4,237 URLs. Search Console fix validation should not be started for these; watch indexed vault URLs instead.
2. **Crawl waste.** `X-Robots-Tag: noindex` on social-card and metadata-logo responses (set in the `handleResponseHeaders` hook, so the `?fallback=` redirects are covered); export-data pages and unnamed exchanges (`Unknown`, `Unknown 0x…`) are `noindex`; `docs` dropped from the static sitemap (it 302s). **After deploy, purge `/social-card/*` and `/metadata-logo/*` in Cloudflare** — successful social cards are cached for a year and edge copies lack the header until purged.
...
5. **Caching.** `/logos/**` (a server endpoint, not `static/`) now `public, max-age=604800, stale-while-revalidate=86400`. Fonts and avatars under `static/` are served by adapter-node's static file server, which runs before the `handle` hook and sets no cache header (Cloudflare then applied its four-hour default). `scripts/server.js` — the production entry point — now sets them through `scripts/static-cache-control.js`: `/fonts/*.woff2` one year immutable (the files never change in place; the stylesheet is renamed on change), `/fonts/*.css` and `/avatars/*` one day. No Cloudflare rule is needed; `vite preview` bypasses `scripts/server.js`, so verify against `node scripts/server.js` or production.
6. **Titles, descriptions, social images.** `SocialCardMetaTags` now builds the title with the brand suffix (`getPageTitle`), truncates the description, fills in Open Graph/Twitter defaults, and is used by every indexable frontend template — the direct `<svelte:head><title>` blocks of the trading-view, glossary, blog, strategy, about, pricing, podcast, search, dataset, community, newsletter and API pages were migrated (the remaining direct titles are strategy sub-pages, developer pages and error pages). Token, pair, exchange and chain pages get data-led descriptions (TVL, volume, pair counts) and the chain social card. Home: `DeFi vault rankings, yields and risk data | Trading Strategy`.
...
- `curl -sI https://tradingstrategy.ai/social-card/trading-strategy | grep -i x-robots` (after the purge) and `…/fonts/NeueHaasGroteskText/55.woff2 | grep -i cache-control` shows the one-year value.
...
### What changed (2026-09-25, branch `seo-round-4`)

1. **Measurement first** — `templates` and `terms` commands and the baseline above.
2. **NewBet** is on the name blocklist (`noindex`, out of the vault sitemap).
3. **Titles in search wording** — hubs are "<Entity> vaults" with an APY qualifier; `/vaults` is "Best DeFi vaults by APY and risk"; `/vaults/stablecoins` is "Compare stablecoin yields"; a chain that shares its name with a protocol (Hyperliquid) says "Vaults on <chain>" so it does not compete with the protocol hub; vault pages name protocol and chain. Hub descriptions are data-led. The protocol hub called the annualised one-month figure an "average monthly return", and vault descriptions showed an absolute one-month return and TVL in denomination units; both are corrected.
4. **Crawlable hub text** — protocol, curator and stablecoin long descriptions were only rendered after "View more"; they are now in the server HTML, collapsed.
5. **Stablecoin yield comparison** — a "Best stablecoin yields right now" table on `/vaults/stablecoins`. The first live run named 600–850 % trading vaults and funds as the best USDC yield, so leaders are yield vaults only (no pools, tokenised funds or trading-strategy tags — the risk rating is protocol risk and does not screen them out), rated Low risk or safer, with ≥ $100k TVL, ≥ 3 months of history and APY ≤ 100 %. A generated "top vaults" paragraph and table on every hub was also built and then removed before release: it repeated the listing below it in a sentence full of qualifiers and gave visitors nothing to act on.
6. **Internal links** — "Similar <protocol> vaults" on vault pages; the glossary terms that rank link to the matching vault pages (`hyperliquid-provider-vault` → HLP vault and Hyperliquid hub); the vault returns section is "<name> APY and returns".
7. **Methodology** — `/vaults/methodology`, linked from every listing.
8. **Structured data** — hub `ItemList` entries for the server-rendered rows; `dateModified` from the dataset.

Deferred by the plan until measured: protocol × stablecoin pilot pages (workstream 4) and crawlable pagination (workstream 9).

After release: request removal of `/vaults/newbet` in Search Console; request indexing of `/vaults/protocols/morpho` and `/vaults/stablecoins/usdc`; rerun `pnpm run seo:search-console terms --inspect` and `templates` at 4 and 8 weeks.
