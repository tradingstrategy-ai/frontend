...
Implemented on branch `seo-round-4` on 2026-09-25 (workstreams 0–3, 5–8, 10 and 11; workstreams 4 and 9 deferred until the results are measured, as planned). Outcomes and deviations are in `docs/google-webmasters.md`, "Round 4". Deviations: the generated hub paragraph and "Why the top … differ" table (workstreams 3 and 11) were built and then removed at review — they repeated the listing below them in qualifier-laden prose and helped no one, so workstream 3's value rests on the server-rendered long descriptions and workstream 11 on the `ItemList` entries; the leader rules gained "yield vaults only" and an APY ceiling of 100 % after a live-data run showed trading vaults as leaders; comparison tables are on every hub, not only Morpho, Hyperliquid and USDC, because they are generated from each hub's own data; workstream 8 needed no HLP-only code (the returns section is "<name> APY and returns" on every vault page and the glossary term links to the HLP vault); hub long descriptions were found to be missing from the server HTML and are now rendered; the NewBet sitemap integration test was not added (the sitemap and loader call the unit-tested `isVaultIndexable()`). Originally: planned. Based on a Search Console API pull (90 days to 2026-09-24, plus before/after windows around the round 1–3 releases), Google Autocomplete for the seed terms below, and the SSR HTML of the live vault templates on 2026-09-25. Reviewed by Codex (`gpt-6-sol`, read-only) on 2026-09-25: its nine findings are folded in below and its seven ideas are workstreams 7–11 and the research item under "Outside this repository". Owner decisions: NewBet goes on the ignore list (workstream 0).
