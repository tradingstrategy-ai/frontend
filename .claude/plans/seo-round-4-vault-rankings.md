...
Implemented on branch `seo-round-4` on 2026-09-25: workstreams 0, 1, 2 and 6; from workstream 3 the server-rendered hub descriptions; from workstream 7 the `/vaults/stablecoins` title, heading and description; from workstreams 5 and 8 the glossary links to the hubs and the HLP vault; from workstream 11 the `ItemList` entries. Workstreams 4 and 9 are deferred until the results are measured, as planned. Outcomes are in `docs/google-webmasters.md`, "Round 4". Built and then removed at the owner's review before release: the generated hub paragraph and "Why the top … differ" table (workstreams 3 and 11), "Similar <protocol> vaults" (5), the stablecoin yield comparison table (7), the "<name> APY and returns" heading (8), and the "How we rank" link with its `/vaults/methodology` page (10). They repeated what the listing already shows, picked trading vaults as "best" or "similar" on live data, renamed a clear heading for search's sake, or sat below an infinitely scrolling table where nobody would see them; see `docs/vault-listings.md`, "Search wording". Other deviations: hub long descriptions were found to be missing from the server HTML and are now rendered; the NewBet sitemap integration test was not added (the sitemap and loader call the unit-tested `isVaultIndexable()`). Originally: planned. Based on a Search Console API pull (90 days to 2026-09-24, plus before/after windows around the round 1–3 releases), Google Autocomplete for the seed terms below, and the SSR HTML of the live vault templates on 2026-09-25. Reviewed by Codex (`gpt-6-sol`, read-only) on 2026-09-25: its nine findings are folded in below and its seven ideas are workstreams 7–11 and the research item under "Outside this repository". Owner decisions: NewBet goes on the ignore list (workstream 0).
...
"Stablecoin vaults" is our wording, not a search phrase: Autocomplete completes it with nothing vault-specific, while "hyperliquid vaults", "morpho vaults", "hlp vault" and "usdc vault" each complete to vault-specific phrases. The listings themselves are stablecoin-denominated only (the table's "Stablecoin-only" note: ETH- and BTC-denominated vaults are excluded), so "stablecoin" is accurate, just not what people type; the H1 and title use the search phrase and the subtitle and `/vaults/methodology` state the scope. (The first Codex review read `/vaults` as unfiltered; the dataset and the table note say otherwise.) Vault detail titles name neither protocol nor chain, although searchers type "<vault> morpho vault". Vault pages have a 1.25 % CTR at position ~8.
