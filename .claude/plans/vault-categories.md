# Vault category listing pages

## Goal

Add a `/vaults/categories` index and `/vaults/categories/[category]` detail pages backed by the new category metadata and strategy tags in `top_vaults_by_chain.json`. Link the index from the shared vault navigation, show the source category description and aggregate metrics, and reuse the existing server-backed vault listing pipeline for each category.

Category rows must be alphabetical by label on first load. Source-controlled category tags use underscores, such as `directional_trading`, while public URLs use dashes, such as `/vaults/categories/directional-trading`.
...
## Vault detail categories

Add category links to every vault detail page that has registered `strategy_tags`:
...
3. Render a concise, accessible line beginning with `Categories:` followed by category-name links separated by literal commas. Each link uses `resolve('/vaults/categories/<dash-slug>')`, with the source label as visible text, so every keyword navigates to its respective category page. Keep the links independently focusable and preserve readable wrapping on narrow screens; a long list must wrap between links without splitting or overlapping the comma separators.
4. Keep this presentation separate from ranking availability: categories display below the ranking position when rankings are present and in the same position when the source has no ranking data. Do not treat the category list as a return ranking or invent category-specific rank values.

## Category index

Create `src/routes/vaults/categories/+page.server.ts` and `+page.svelte` following the protocols/chains index structure where it remains semantically valid:
...
- Show the columns Category, Description, Vaults, Avg. APY (30d), and TVL using the existing formatters and responsive table/card behaviour. Do not add an unused sortable Description header.
...
Create `src/routes/vaults/categories/[category=slug]/+page.server.ts` and `+page.svelte`:
...
3. Render the heading as `${category.label} vaults` and render `category.description` in-page with the shared Markdown component. Use a plain-text, category-specific sentence for HTML/social metadata and JSON-LD so Markdown links are not exposed as markup syntax.
4. Add a small category description/summary component using the existing `MetricsBox` visual pattern. Alongside the Markdown description, state the source `vault_count`, `tvl_usd`, and source-provided 30-day average `one_month_apy` when present. Label these as dataset-wide category figures so they are not confused with the filter-sensitive listing summary shown around the table. Only describe the APY as TVL-weighted if that producer-side contract is documented and verified during implementation.
...
- Add `{ href: '/vaults/categories', label: 'Categories' }` to the primary links in `src/lib/top-vaults/VaultListingsSelector.svelte`. Verify wrapping and dropdown positioning at desktop, tablet, and mobile widths after adding the extra item.
- Extend `src/routes/vaults/sitemap.xml/+server.ts` with the category index and one dash-form detail URL for every key in the top-level category map, including zero-vault categories. Sitemap and canonical URLs must never expose source underscores.
- Do not add categories to global vault search in this change; the request is for vault navigation and listing pages, and search currently has its own entity ranking and payload contract.
...
- Vault detail category labels and links: a multi-tag vault renders alphabetically ordered, comma-separated links directly below rankings; null, absent, and unregistered tags render no line; every link targets the matching dash-form category route; and the line wraps without overlap on mobile.
- The category index's five columns, Markdown link rendering, zero-vault row, alphabetical default, URL-backed alternate sorting, and responsive layout without horizontal page overflow.
- Playwright visual checks of the category index at desktop, tablet, and mobile viewports. Capture screenshots at each viewport and assert that the desktop table remains readable, tablet content does not collide or clip, the mobile card layout preserves category name, description, metrics, and links, and the added vault navigation item wraps without obscuring the page heading or table controls.
- Category detail title, description, dataset-wide stats, filtered listing summary, mini-chart, multi-tag membership, valid zero-vault/zero-chart-data states, unknown-category 404, dash-form route matching, legacy underscore-to-dash redirect, and progressive loading that remains in scope.
...
Update `docs/vault-data-source.md` with the `categories` and `strategy_tags` contracts and `docs/vault-listings.md` with the new `category` listing key/scope and the distinction between source aggregates and interactive listing summaries.
...
6. Capture and review Playwright screenshots of `/vaults/categories` at representative desktop, tablet, and mobile viewports. Confirm that all content remains visible and legible, no elements overlap or cause horizontal page overflow, and table/card transformations retain the description and all required metrics. Store any intentional regression baselines with the relevant integration-test screenshots rather than relying only on DOM assertions.

## Acceptance criteria

- The vault navigation contains a working Categories link.
- `/vaults/categories` lists every valid source category alphabetically by label by default with its rendered description, TVL, vault count, and 30-day average APY.
- `/vaults/categories/<dash-slug>` renders the category title and description, source aggregate summary, scoped vault listing, and TVL/returns chart when data exists.
- Vault detail pages show their registered category names immediately below the ranking area as comma-separated links to the matching category pages.
- Multi-tag vaults appear in every matching category; absent or null tags match none.
- Known zero-vault categories render successfully, while unknown category keys return 404.
- Filtering, sorting, initial pagination, and continuation requests cannot escape the category scope.
- Category URLs are dash-form, canonical, and discoverable in the vault sitemap; underscore source tags never appear in public URLs.
- The category index is visually verified with Playwright screenshots on desktop, tablet, and mobile, with no clipping, overlap, unreadable Markdown descriptions, or horizontal page overflow.
