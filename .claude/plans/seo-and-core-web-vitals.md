...
5. Pair URLs are emitted by the backend sitemaps (`api/sitemap/pairs/paged/N.xml`, listed from `src/routes/sitemap.xml/+server.ts`). Open a backend ticket to apply the same thresholds there; a `noindex` page that is still in a sitemap generates a Search Console warning but is otherwise harmless. Token pages are not in any sitemap.
