...
Implemented on 2026-09-18 and opened as one PR, [#1444](https://github.com/tradingstrategy-ai/frontend/pull/1444) on branch `test-suite-cleanup`, whose first three commits are the plan's PR A (workstream 1, 2.1–2.3), PR B (2.4–2.6) and PR C (2.7, workstream 3, `docs/tests.md`) — each was verified green on its own with `--workers=2` before the branches were stacked — followed by a simplification pass and the fixes from Grok's xhigh review of the PR. Measured: integration 385 → 342 → 295 → 290 test results (300 after the review split the Charts active-state check into one test per page), aggregate 329 s → 230 s → 200 s → 203 s; unit 659 → 649 tests, ~10.5 s → ~4 s, no network I/O. Deviations from the plan: `charts-dropdown` kept two mobile tests (merging them made the reopened menu's items unstable on a 375 px viewport) and its `openChartsMenu` retry loop was replaced by the hydration wait; the 11 chart links moved into `src/lib/top-vaults/vault-chart-links.ts`, shared by `VaultListingsSelector`, `ScatterPlotSelector` and the test; `yield-protocol` to 1 and `yield-risk` to 2 (legend checks folded into the render test); `token-details` left as is; `chain-details.test.ts` was rewritten for the current page rather than made to pass as written. Two hydration races surfaced when per-assertion tests were merged (`diagnostics`, `datasets`) — a value `fill()`ed before hydration is wiped on re-render — fixed by waiting for the header's `data-navigation-hydrated` marker. The one remaining local failure is the pre-existing `vaults/index.test.ts` "falls back to vault sorting…" flake. Originally: planned. Reviewed by Grok (grok-4.6) on 2026-09-18 in read-only mode; its eleven findings (three high) are folded in below — the main corrections were that `.scatter-plot-selector` is a different component from the Charts dropdown, that `tests/e2e/trading-view/chain-details.ts` should be renamed rather than deleted, that the vault-detail block in `vaults/index.test.ts` was mis-described and does not overlap `VaultTransactionStatus.test.ts`, and that the target counts were optimistic. Based on a full audit of `src/**/*.test.ts`, `tests/integration` and `tests/e2e` on master at `192e977a`, with runtime measured locally (unit: Vitest JSON reporter; integration: Playwright JSON reporter, 4 workers) and cross-checked against the CI run of 2026-09-18 08:55 UTC (`test` job: build 1 m 13 s, unit + integration 4 m 13 s; `test-e2e` job: 19 s of tests).
