<!--
Historical vault TVL by stablecoin page with a server-side aggregated weekly stacked line chart.
-->
<script lang="ts">
	...
	import type { HistoricalTvlByStablecoinPayload } from '$lib/echarts/historical-tvl';
	...
	import HistoricalTvlStablecoinChart from './HistoricalTvlStablecoinChart.svelte';

	let chartData = $state<HistoricalTvlByStablecoinPayload | null>(null);
...
<main class="historical-tvl-stablecoin-page">
	...
	<Section tag="header">
		...
		<HeroBanner>
			{#snippet subtitle()}
				...
				<a href={resolve('/glossary/total-value-locked-tvl')}>TVL</a> has evolved across different stablecoins over time.
			{/snippet}
			{#snippet title()}
				<span>Historical vault TVL by stablecoin</span>
	...
	<Section padding="sm" class="chart-section">
		<HistoricalTvlStablecoinChart data={chartData} dataLoading={chartLoading} error={chartError} />
		<ScatterPlotSelector />
	</Section>
...
<style>
	.historical-tvl-stablecoin-page {
		:global(.badge) {
			font-size: 0.44em;
	...
	:global(.chart-section .standalone-historical-tvl-stablecoin-shell .chart-surface) {
