<!--
@component
Client-side ECharts stacked line chart for historical vault TVL by chain.
...
<script lang="ts">
	...
	const searchParamsSchema = {
		chains: { type: 'string', defaultValue: '' }
	...
	let selectedSeriesKeys = $derived(urlState.chains ? urlState.chains.split(',').filter(Boolean) : []);
	...
	function getLegendLogoUrl(chainIds: number[]) {
		const chainSlug = chainIds.map((chainId) => getChain(chainId)?.slug).find(Boolean);
	...
	function toggleSeries(key: string) {
		...
		if (selectedSeriesKeys.includes(key)) {
			if (selectedSeriesKeys.length <= 1) {
				...
				updateUrl({ chains: selectedSeriesKeys.filter((itemKey) => itemKey !== key).join(',') });
			}
			return;
		}

		const next = allKeys.filter((itemKey) => [...selectedSeriesKeys, key].includes(itemKey));
		updateUrl({ chains: next.length === allKeys.length ? '' : next.join(',') });
	}
	...
	function buildTooltip(
		...
		return [
			...
			...(hiddenCount > 0
				? [
						`<div style="margin-top: 0.45rem; color: #94a3b8; font-family: ${chartFontFamily};">+${hiddenCount} more chains</div>`
					]
				: [])
		].join('');
	}
	...
	async function renderChart() {
		...
		const series = visibleSeries.map((item, index) => {
			const colour = protocolPalette[index % protocolPalette.length];
			return {
				name: item.label,
				type: 'line',
				stack: 'vault-tvl',
...
<div class="historical-tvl-chain-chart standalone-historical-tvl-chain-shell" data-testid="vault-scatter-plot">
	{#if data && !dataLoading && !effectiveError && hasSeries}
		<div class="chain-selector" aria-label="Chain selector">
			{#each data.series as item, index (item.key)}
				...
				>
					{#if getLegendLogoUrl(item.chainIds)}
						<img class="chain-chip-logo" src={getLegendLogoUrl(item.chainIds)} alt="" loading="lazy" />
					{:else}
						<span class="chain-chip-logo chain-chip-logo-fallback" aria-hidden="true"></span>
					{/if}
					<span class="chain-chip-label">{item.label}</span>
					<span
						class="chain-chip-dot"
						aria-hidden="true"
						style={`background: ${protocolPalette[index % protocolPalette.length]};`}
					></span>
				</button>
			{/each}
		</div>
	...
	<div class="chart-surface">
		{#if dataLoading}
			<div class="chart-state">
				<Spinner />
				<p>Loading historical vault TVL by chain…</p>
			</div>
	...
	{#if data && !dataLoading && !effectiveError && visibleSeries.length > 0}
		<p class="chart-summary">Today selected chains have total {formatUsd(latestSelectedTvl)} TVL</p>
	{/if}
