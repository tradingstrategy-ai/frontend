<svelte:head>
	<title>TradingStrategy.ai community resources</title>
	<meta name="description" content="Resources to learn algorithmic trading for crypto" />
</svelte:head>

<main>
	<section>
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<h1>Community</h1>
				</div>
			</div>

			<div class="row">
				<div class="col-md-4 mb-4">
					<div class="card bg-primary shadow-soft border-light text-center h-100">
						<div class="card-body">
							<h3 class="h5 card-title">
								<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250 250">
									<g transform="matrix(10.416666666666666,0,0,10.416666666666666,0,0)"
										><g>
											<path
												d="M8.58,18.82a2,2,0,0,1-.32.17,6.48,6.48,0,0,1-.63.16.35.35,0,0,0-.28.39.34.34,0,0,0,.4.28,4.17,4.17,0,0,0,.73-.13A2.18,2.18,0,0,0,9,19.48,2.54,2.54,0,0,0,10.29,17a2.77,2.77,0,0,0-4.24-2.1,2.7,2.7,0,0,0-1.26,2.16,2.58,2.58,0,0,0,.49,1.69,2.49,2.49,0,0,0,1.43.9.29.29,0,0,0,.37-.21.29.29,0,0,0-.2-.37,1.76,1.76,0,0,1-1-.75,1.61,1.61,0,0,1-.2-1.15,1.77,1.77,0,0,1,.91-1.3,1.81,1.81,0,0,1,1.53-.13A1.83,1.83,0,0,1,9.3,17.13,1.69,1.69,0,0,1,8.58,18.82Z"
												style="fill: #458b00;fill-rule: evenodd"
											/>
											<path
												d="M17.54,10.22a15.18,15.18,0,0,1,2.53,8.6.35.35,0,0,0,.33.36.36.36,0,0,0,.36-.33A15.66,15.66,0,0,0,18.47,9.6a11.91,11.91,0,0,0-3.4-3.5A13.49,13.49,0,0,0,10.6,4.21a15.51,15.51,0,0,0-2.42-.33,14,14,0,0,0-2.43.06.3.3,0,0,0-.28.33.31.31,0,0,0,.33.27,12,12,0,0,1,2.31.08,14.13,14.13,0,0,1,2.26.45,13.16,13.16,0,0,1,4.05,2A12.76,12.76,0,0,1,17.54,10.22Z"
												style="fill: #458b00;fill-rule: evenodd"
											/>
											<path
												d="M12.7,13.66a9.37,9.37,0,0,1,1.63,5.25.35.35,0,0,0,.33.36.35.35,0,0,0,.35-.34A9.8,9.8,0,0,0,13.64,13a7.53,7.53,0,0,0-2.19-2.24A8.13,8.13,0,0,0,8.58,9.63,9.4,9.4,0,0,0,7,9.44,9.78,9.78,0,0,0,5.5,9.5a.31.31,0,0,0-.28.33.3.3,0,0,0,.33.27A7.72,7.72,0,0,1,7,10.18a7.51,7.51,0,0,1,1.38.31,8.4,8.4,0,0,1,2.45,1.23A8.27,8.27,0,0,1,12.7,13.66Z"
												style="fill: #458b00;fill-rule: evenodd"
											/>
											<path
												d="M24,8.45c0-2-.1-3.81-.24-5.06a3.93,3.93,0,0,0-.66-1.82,2.58,2.58,0,0,0-1.76-1,80.71,80.71,0,0,0-11-.47A37.17,37.17,0,0,0,3,.76a4.54,4.54,0,0,0-1.67,1,3,3,0,0,0-1,1.73A47.12,47.12,0,0,0,0,10.67,96.32,96.32,0,0,0,.74,21.44a2.44,2.44,0,0,0,1,1.48,4.87,4.87,0,0,0,2.23.85,28.14,28.14,0,0,0,4.26.17c3.17-.05,7-.33,9.77-.39a.35.35,0,0,0,.34-.35.35.35,0,0,0-.35-.34c-2.79,0-6.6.26-9.77.26a28,28,0,0,1-4.11-.21,4.52,4.52,0,0,1-1.56-.53,1.63,1.63,0,0,1-.86-1.09,84.51,84.51,0,0,1-.55-8.85,55.48,55.48,0,0,1,.32-8.81,1.84,1.84,0,0,1,.64-1.07,3.24,3.24,0,0,1,1.18-.69,37.36,37.36,0,0,1,7.1-.65,82.66,82.66,0,0,1,10.82.36,1.57,1.57,0,0,1,1.09.61,2.76,2.76,0,0,1,.49,1.32c.16,1.22.27,3,.33,5,.14,4.21.1,9.28.13,11.71a3.49,3.49,0,0,1-.38,1.67,1.45,1.45,0,0,1-1.18.9l-2,.07a.29.29,0,0,0-.3.3.3.3,0,0,0,.3.3l2,0a1.92,1.92,0,0,0,1.54-.85,4.07,4.07,0,0,0,.73-2.35C23.92,17.74,24,12.66,24,8.45Z"
												style="fill: #cc0000;fill-rule: evenodd"
											/>
										</g></g
									></svg
								>
								Blog
							</h3>

							<p class="card-text">
								Follow Trading Strategy protocol development and gain insights in the decentralised
								trading.
							</p>

							<p>
								<span class="badge text-uppercase">Coming soon</span>
							</p>
						</div>

						<div class="card-footer">
							<button disabled class="btn btn-primary btn-sm">Read blog </button>
						</div>
					</div>
				</div>

				<div class="col-md-4 mb-4">
					<div class="card bg-primary shadow-soft border-light text-center h-100">
						<div class="card-body">
							<h3 class="h5 card-title">
								<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250 250">
									<g transform="matrix(10.416666666666666,0,0,10.416666666666666,0,0)"
										><g>
											<path
												d="M18.78,9.62c-.11-.39-.23-.79-.36-1.17S18.14,7.68,18,7.3c-.07-.17-.14-.4-.24-.6a1.28,1.28,0,0,0-.24-.31A2.1,2.1,0,0,0,16.77,6a5.11,5.11,0,0,0-1.11-.25c-.6-.09-1.23-.15-1.85-.19-1.43-.07-1.27,0-3,0a8.55,8.55,0,0,0-1,.06l-1.35.29a7.13,7.13,0,0,0-1.36.47,1.6,1.6,0,0,0-.58.45,4.53,4.53,0,0,0-.75,1.65c-.21.84-.28,1.76-.41,2.5a12.57,12.57,0,0,0-.49,2.39,1.63,1.63,0,0,0,.57,1.26,3.49,3.49,0,0,0,1.14.61,6.39,6.39,0,0,0,1.77.31,4.45,4.45,0,0,0,.81,0,.8.8,0,0,0,.44-.23,2,2,0,0,0,.35-.5c.16-.32.26-.7.39-1,0,0,0,0,0-.06.32,0,.64.12,1,.15a3.26,3.26,0,0,0,.7,0l1.5-.18.21-.05a.94.94,0,0,1,.12.18,5.53,5.53,0,0,1,.36.79c0,.09.06.31.11.44a.79.79,0,0,0,.28.35,1.68,1.68,0,0,0,.61.21,6.68,6.68,0,0,0,1.08,0,7.67,7.67,0,0,0,1.94-.29,1.86,1.86,0,0,0,.89-.57,1.22,1.22,0,0,0,.24-.54,5.38,5.38,0,0,0,.06-.94,8.8,8.8,0,0,0-.11-1.54C19.16,11.07,19,10.35,18.78,9.62Zm-.41,4.27a.59.59,0,0,1-.07.29,1.19,1.19,0,0,1-.64.29,8,8,0,0,1-1.44.13c-.16,0-.41,0-.65,0l-.21,0a1.93,1.93,0,0,0-.15-.35,8.25,8.25,0,0,0-.42-.92,2.43,2.43,0,0,0-.51-.63.59.59,0,0,0-.4-.14,4,4,0,0,0-.58.09l-1.38.17a2.79,2.79,0,0,1-.5,0A12,12,0,0,0,10,12.61a.62.62,0,0,0-.55.29,3,3,0,0,0-.15.48c-.1.25-.18.55-.3.83a1.24,1.24,0,0,1-.1.2c-.1-.07-.42.06-.51.05a6.2,6.2,0,0,1-1.27-.15A2.74,2.74,0,0,1,6,13.84a.84.84,0,0,1-.22-.78c.05-.69.33-1.45.4-1.94.1-.73.14-1.63.32-2.45a3.72,3.72,0,0,1,.55-1.39A1.27,1.27,0,0,1,7.56,7a7.22,7.22,0,0,1,1-.38l1.26-.31a7.14,7.14,0,0,1,.93-.08c.17,0,2.14,0,3,.05.61,0,1.2.13,1.78.23a4.39,4.39,0,0,1,.95.23A1.49,1.49,0,0,1,17,7l.26.64a11.07,11.07,0,0,1,.39,1.1c.11.37.22.74.31,1.13a18.58,18.58,0,0,1,.42,2.1,7.82,7.82,0,0,1,.07,1.37C18.39,13.46,18.39,13.68,18.37,13.89Z"
												style="fill: #458b00;fill-rule: evenodd"
											/>
											<path
												d="M10,8.26a1.74,1.74,0,0,0-.45-.06A1.69,1.69,0,0,0,8,10.31a1.39,1.39,0,0,0,1.58,1.18A1.75,1.75,0,0,0,11.1,10,1.48,1.48,0,0,0,10,8.26ZM10.1,9.8a.72.72,0,0,1-.61.58c-.47.08-.62-.41-.58-.81.1-.8.59-.65.61-.65A1.05,1.05,0,0,1,9.76,9,.66.66,0,0,1,10.1,9.8Z"
												style="fill: #458b00;fill-rule: evenodd"
											/>
											<path
												d="M14.9,8.26c-2-.35-2.1,1.55-1.78,2.25a1.41,1.41,0,0,0,1.86.7,1.87,1.87,0,0,0,1-1,1.3,1.3,0,0,0-.25-1.5A1.72,1.72,0,0,0,14.9,8.26Zm.17,1.51a.85.85,0,0,1-.46.38c-.47.2-.75-.16-.76-.54,0-1.06,1-.67,1.24-.29C15.21,9.46,15.14,9.63,15.07,9.77Z"
												style="fill: #458b00;fill-rule: evenodd"
											/>
											<path
												d="M23.16,3.7a4.68,4.68,0,0,0-1-2.41A2.91,2.91,0,0,0,20.71.43,10.34,10.34,0,0,0,18.5.22C16.7.14,15.66.05,14.43,0c-.77,0-1.61,0-2.74,0s-2,.11-3,.22-2,.25-3,.44a15.51,15.51,0,0,0-1.89.39,3.3,3.3,0,0,0-1.29.7A4.16,4.16,0,0,0,1.35,3.5,10.11,10.11,0,0,0,.86,6.3C.76,8,.71,9.64.75,11.33a36.58,36.58,0,0,0,.45,5,17.15,17.15,0,0,0,.54,3.23,2.57,2.57,0,0,0,1,1.28,2.82,2.82,0,0,0,1.12.39,21.76,21.76,0,0,0,2.42,0c.57,0,1.25,0,2,0,7.64-.22,8.12.11,8.13-.79V19l.06.05c.37.35.73.75,1.07,1.09,1,.86,1.86,1.78,2.79,2.67.41.38.83.75,1.26,1.12s.75-.44.75-.46c.08-.47.13-1.28.18-2.27.14-2.89.19-7.32.3-9l.34-4.86A16,16,0,0,0,23.16,3.7Zm-.92,3.59L22,12.17c-.09,1.64-.07,6.08-.17,9,0,.79-.06,1.44-.1,1.91-.9-1-1.73-2-2.62-2.95-.25-.27-.51-.54-.78-.8s-.72-.76-1.14-1.1a3.68,3.68,0,0,0-1-.61.48.48,0,0,0-.61.15.49.49,0,0,0-.11.3c0,.07.05.22.05.3l0,.56.17,1.45c-2,0-5.06.15-7.46.17-.73,0-1.4,0-2,0s-1.34.05-2,0a2.21,2.21,0,0,1-1.09-.32,2,2,0,0,1-.75-1.27,26.65,26.65,0,0,1-.31-2.68,34.82,34.82,0,0,1-.35-4.91c0-1.65.06-3.31.18-4.94a9.45,9.45,0,0,1,.44-2.49,3.13,3.13,0,0,1,.82-1.27,2.25,2.25,0,0,1,.88-.46,15.14,15.14,0,0,1,1.75-.36c1-.18,2-.32,2.93-.43s2-.18,3-.22,1.93,0,2.68,0c1.21,0,2.26.1,4.05.15a9.4,9.4,0,0,1,2,.16,1.79,1.79,0,0,1,1,.54,3.77,3.77,0,0,1,.81,1.89A14.72,14.72,0,0,1,22.24,7.29Z"
												style="fill: #cc0000;fill-rule: evenodd"
											/>
										</g></g
									></svg
								>
								Discord chat
							</h3>

							<p class="card-text">
								Argue what's the best algorithm with fabulous Trading Strategy community members
								(and secretly wish for an airdrop).
							</p>
						</div>

						<div class="card-footer">
							<a rel="external" href="https://discord.gg/yReMpKykaS" class="btn btn-primary btn-sm"
								>Open discussion</a
							>
						</div>
					</div>
				</div>

				<div class="col-md-4 mb-4">
					<div class="card bg-primary shadow-soft border-light text-center h-100">
						<div class="card-body">
							<h3 class="h5 card-title">
								<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250 250">
									<g transform="matrix(10.416666666666666,0,0,10.416666666666666,0,0)"
										><g>
											<path
												d="M11.77,4.26c.71-.6.7-1.21.38-1.62a1.63,1.63,0,0,0-1.94-.34l-.09,0a.37.37,0,0,0-.13.15,1.42,1.42,0,0,0,.25,1.82A1.24,1.24,0,0,0,11.77,4.26Zm-1.16-.9a.81.81,0,0,1-.11-.45,1,1,0,0,1,.65.11c.25.12.09.24-.05.34S10.7,3.52,10.61,3.36Z"
												style="fill: #458b00"
											/>
											<path
												d="M14.94,21.65c.54-.6.49-1.15.15-1.5-1.22-1.31-3,.17-1.8,1.57C13.64,22.13,14.24,22.25,14.94,21.65Zm-.72-1c.15.07,0,.16,0,.22s-.26.19-.36-.05a1.07,1.07,0,0,1-.07-.19A.59.59,0,0,1,14.22,20.62Z"
												style="fill: #cc0000"
											/>
											<g>
												<path
													d="M22.63,9.05a3.13,3.13,0,0,0-1.77-1.86A5.25,5.25,0,0,0,20.23,7a4.74,4.74,0,0,0-.65-.16,5.9,5.9,0,0,0-.72-.09c-1.32.18-1.13.6,0,.71l.57.1L20,7.7c.18.07.36.14.54.23a2.29,2.29,0,0,1,1.24,1.43,9.76,9.76,0,0,1,.15,3.25,10.29,10.29,0,0,1-.26,2.28,4.15,4.15,0,0,1-1,1.87,3.74,3.74,0,0,1-2.09,1,22.67,22.67,0,0,1-3.94.25l-1.19,0c-.39,0-.78,0-1.17-.06a.29.29,0,0,0-.33.25.31.31,0,0,0,.26.33c.32,0,.65.08,1,.1.49,0,1,.06,1.45.08.66,0,1.41.07,2.18.09,0,0,0,.06,0,.1a8.64,8.64,0,0,1-.2,2A2.86,2.86,0,0,1,16,22a1.85,1.85,0,0,1-.66.45,4.74,4.74,0,0,1-1.34.3,8.51,8.51,0,0,1-2.89.06,3.92,3.92,0,0,1-1.67-.68,3.1,3.1,0,0,1-1-1.49,5.47,5.47,0,0,1-.27-1.13c-.35-1.58-.92-.87-.77.12a7.69,7.69,0,0,0,.24,1.27,4,4,0,0,0,1.22,2,4.64,4.64,0,0,0,2.09,1,9,9,0,0,0,3.27-.1,6.27,6.27,0,0,0,1.58-.48,2.64,2.64,0,0,0,.94-.76,3.61,3.61,0,0,0,.64-1.43,8.78,8.78,0,0,0,.09-2.18s0-.06,0-.09a10.12,10.12,0,0,0,1.36-.1,4.66,4.66,0,0,0,2.7-1.23,4.74,4.74,0,0,0,1.13-1.84A9.59,9.59,0,0,0,23,12.62,10.59,10.59,0,0,0,22.63,9.05Z"
													style="fill: #458b00"
												/>
												<path
													d="M8.05,18.33a12,12,0,0,1,.31-2.65A1.29,1.29,0,0,1,9,14.89a3.69,3.69,0,0,1,1-.36c1.64-.36,3.47,0,5-.7a4.34,4.34,0,0,0,2.34-2.93A27,27,0,0,0,17.68,4a6.35,6.35,0,0,0-.25-1.13A3.93,3.93,0,0,0,15.15.58,8.56,8.56,0,0,0,10.52.13a6.7,6.7,0,0,0-1.39.4,3.11,3.11,0,0,0-.86.56,2.85,2.85,0,0,0-.81,1.63,12,12,0,0,0,0,2.53.31.31,0,0,0,.31.28A.3.3,0,0,0,8,5.22a10.79,10.79,0,0,1,.06-2.39,2.22,2.22,0,0,1,.64-1.25,2.81,2.81,0,0,1,.88-.49,6.3,6.3,0,0,1,1-.25,7.79,7.79,0,0,1,4.17.5,3,3,0,0,1,1.75,1.84,4.44,4.44,0,0,1,.19.95,9.77,9.77,0,0,1,0,1.53,19.68,19.68,0,0,1-.48,4.93,3.22,3.22,0,0,1-1.74,2.23c-1.5.66-3.26.28-4.82.62a4.87,4.87,0,0,0-1.32.48,2.37,2.37,0,0,0-1.1,1.43,7.29,7.29,0,0,0-.23,1.91c-.11.29-.64.26-.72.27a7.57,7.57,0,0,1-1.66,0,4.06,4.06,0,0,1-1.52-.4,1.63,1.63,0,0,1-.66-.8,6.28,6.28,0,0,1-.37-1.2,12.2,12.2,0,0,1-.22-2.47,14,14,0,0,1,.29-2.47,5.71,5.71,0,0,1,.42-1.31,1.83,1.83,0,0,1,.49-.65A3.27,3.27,0,0,1,4,7.77a8,8,0,0,1,1.48-.48,15,15,0,0,1,2.43-.37c1.26-.1,2.53-.1,3.75-.1A.32.32,0,0,0,12,6.5a.33.33,0,0,0-.33-.33c-1.23,0-2.53-.06-3.81,0a17.59,17.59,0,0,0-2.55.34A10.47,10.47,0,0,0,3.64,7a4.16,4.16,0,0,0-1,.6,2.71,2.71,0,0,0-.72.9A6.43,6.43,0,0,0,1.37,10,14.08,14.08,0,0,0,1,12.68a11.52,11.52,0,0,0,.19,2.67,5.72,5.72,0,0,0,.55,1.72,2.43,2.43,0,0,0,.94,1,4.46,4.46,0,0,0,1.52.5,7.63,7.63,0,0,0,2,.06Z"
													style="fill: #cc0000"
												/>
											</g>
										</g></g
									></svg
								>

								Source and development
							</h3>

							<p class="card-text">
								Explore Trading Strategy Github repository source code and strategy notebooks.
							</p>
						</div>

						<div class="card-footer">
							<a
								rel="external"
								href="https://github.com/tradingstrategy-ai/"
								class="btn btn-primary btn-sm">Dive into code</a
							>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
</main>

<style>
	h1 {
		margin: 30px 0;
	}

	.card {
	}

	svg {
		width: 64px;
		display: block;
		margin: 5px auto;
	}

	.btn[disabled] {
		cursor: not-allowed;
	}
</style>
