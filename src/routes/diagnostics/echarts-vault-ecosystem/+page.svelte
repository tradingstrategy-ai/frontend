...
<script lang="ts">
	...
	const ECHARTS_AXIS_FONT_STACK = "'IBM Plex Sans', 'Segoe UI', 'Helvetica Neue', Arial, sans-serif";
	const TOOLTIP_TITLE_STYLE =
		'margin-bottom: 0.35rem; color: #ffffff; font-size: 1.05rem; font-weight: 700; line-height: 1.35;';
	const TOOLTIP_BODY_STYLE = 'color: #d5deea;';
	const TOOLTIP_HINT_STYLE = 'margin-top: 0.55rem; color: #f8fafc;';
...
<style>
	...
	.chart-shell {
		...
		background:
			radial-gradient(circle at top left, color-mix(in srgb, #22c55e, transparent 96%), transparent 42%),
			linear-gradient(180deg, color-mix(in srgb, var(--c-box-2), transparent 16%), transparent 72%),
			color-mix(in srgb, var(--c-box-1), transparent 10%);
		box-shadow: inset 0 1px 0 color-mix(in srgb, var(--c-text-light), transparent 95%);
		overflow: hidden;
