...
<script lang="ts">
	...
	$: priceChangeColorClass =
		summary.price_change_24h >= 0 ? 'price-change-green' : 'price-change-red';
...
<svelte:head>
	...
	<meta
		name="description"
		content={'Price chart and technical analysis for trading ' +
			summary.pair_symbol +
			' on ' +
...
<div class="container">
	...
	<p>
		<strong>{summary.pair_name}</strong> trades as <strong>{summary.pair_symbol}</strong> on
		<a class="body-link" href="/{chain_slug}/{exchange_slug}">{details.exchange_name}</a>
		on <a class="body-link" href="/{chain_slug}">{details.chain_name}</a>.
	</p>

	<p>
		The price of <strong>{summary.base_token_symbol_friendly}</strong> in
		<strong>{summary.pair_symbol}</strong>
		pair is <strong class={priceChangeColorClass}>{formatDollar(summary.usd_price_latest)}</strong>
		and is
	...
	<p>
		...
		<strong><Time relative timestamp={Date.parse(details.first_trade_at)} /></strong>.
	</p>
	...
	<div class="chart-wrapper">
		<CandleStickChart title={summary.pair_name + ' converted to USD'} bind:candles />
	</div>

	<h2>Price and liquidity movement</h2>

	<p>
		The price and liquidity of <strong>{summary.base_token_symbol_friendly}</strong> in this trading
		pair. The amounts are converted to US dollar through
