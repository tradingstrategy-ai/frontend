 ...
 */
export function toListingStrategy(config: YamlStrategyConfig, vault?: VaultInfo): ConnectedStrategyInfo {
	const now = new Date();
	...
	return {
		...
		started_at: vault?.start_date ? new Date(vault.start_date) : now,
		...
		summary_statistics: {
			calculated_at: now,
			launched_at: vault?.start_date ? new Date(vault.start_date) : null,
			first_trade_at: null,
			last_trade_at: null,
			enough_data: true,
			...
			key_metrics: {
				...
				started_at: vault?.start_date
					? {
							kind: 'started_at' as const,
							source: 'live_trading',
							value: new Date(vault.start_date).getTime() / 1000,
							calculation_method: 'latest_value',
							calculation_window_start_at: null,
							calculation_window_end_at: now,
							help_link: null
