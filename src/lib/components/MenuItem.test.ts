...
describe('MenuItem component', () => {
	test('renders a crawlable link', () => {
		render(MenuItem, { label: 'Vault ranking', targetUrl: '/vaults' });
		const link = screen.getByRole('link', { name: 'Vault ranking' });
		expect(link).toHaveAttribute('href', '/vaults');
		expect(link).not.toHaveAttribute('aria-current');
	});

	test('keeps the href on the active item and marks it as the current page', () => {
		render(MenuItem, { label: 'Vault ranking', targetUrl: '/vaults', active: true });
		const link = screen.getByRole('link', { name: 'Vault ranking' });
		expect(link).toHaveAttribute('href', '/vaults');
