...
test.describe('historical vault TVL by stablecoin chart data endpoint', () => {
	test('returns cached JSON payload with aligned series data', async ({ request }) => {
		const response = await request.get('/trading-view/vaults/historical-tvl-stablecoin/chart-data');
		...
		for (const series of data.series) {
			...
			expect(series.stablecoinSlug).toEqual(expect.any(String));
			expect(series.values).toHaveLength(data.weeks.length);
		}
	});
});

test.describe('historical vault TVL by stablecoin page', () => {
	test.beforeEach(async ({ page }) => {
		await page.goto('/trading-view/vaults/historical-tvl-stablecoin');
	});
